...
#Remote Path
	...
	echo "get $fileName" |sftp $username@$ftp_site:$rpath/$fileName /var/www/magentodev/var/import/products/current.zip
	echo "get $fileNameStock" |sftp $username@$ftp_site:$rpath/$fileNameStock /var/www/magentodev/var/import/avilability/current.csv
	
#	echo $rpath/$fileName " and " $rpath/$fileNameStock "copied to" $spath
